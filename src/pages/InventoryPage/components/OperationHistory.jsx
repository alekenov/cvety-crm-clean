...
import { ArrowDown, ArrowUp, User, ExternalLink, ChevronDown, ChevronUp, Trash, ClipboardCheck } from 'lucide-react';
...
export default function OperationHistory({ history }) {
  ...
  const getOperationColor = (type) => {
    switch(type) {
      ...
      case 'revision': return 'text-purple-600';
  ...
  const getOperationIcon = (type) => {
    switch(type) {
      ...
      case 'revision': return <ClipboardCheck size={16} />;
      default: return null;
    }
  };
  ...
  const getOperationName = (type) => {
    switch(type) {
      case 'in': return 'Приемка';
      case 'out': return 'Продажа';
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-2">
        <div className="flex justify-between items-start">
          <div className="flex-grow">
            ...
            {operation.comment && (
              <p className="text-sm text-gray-600 mt-2">{operation.comment}</p>
            )}
                      ...
                      {operation.items.map(item => (
                        <div key={item.id} className="flex justify-between text-sm">
                          ...
                          <span className="text-gray-600">
                            {item.quantity > 0 ? '+' : ''}{item.quantity} шт × {item.price} ₸
                          </span>
                        </div>
                      ))}
                    </div>
    ...
    <div className="bg-white rounded-lg shadow-sm p-4">
      <div className="bg-white rounded-lg mb-4">
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => setSelectedOperation('revision')}
            className={`px-3 py-1 rounded-full text-sm ${
              selectedOperation === 'revision' ? 'bg-purple-100 text-purple-600' : 'bg-gray-100'
            }`}
          >
            Ревизии
          </button>
